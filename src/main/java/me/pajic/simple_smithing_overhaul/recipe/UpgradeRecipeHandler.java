        ...
        if (ModUtil.enchantmentUpgradingEnabled()) {
            ...
            if (itemEnchantments != null) {
                ...
                if (lapisAmount <= itemEnchantments.entrySet().size() && slots.get(1).getItem().getCount() == 1) {
                    ...
                    itemEnchantments.addToTooltip(Item.TooltipContext.of(level), consumer, TooltipFlag.NORMAL/*? >=26.1 {*/, updatedStack.getComponents()/*?}*/);
        ...
        if (SSO.CONFIG.pinnacleEnchantment.enablePinnacleEnchantment.get()) {
            if (ModUtil.isPinnacleEnchantmentRecipe(slots)) {
                ...
                if (slots.get(1).getItem().getCount() == 1) {
                    ...
                    if (itemEnchantments.stream().allMatch(ei -> ModUtil.level(ei) >= ModUtil.enchantment(ei).value().getMaxLevel() && !registry.getOrThrow(EnchantmentTags.CURSE).contains(ModUtil.enchantment(ei)))) {
                        ...
                        maxedOutEnchantments.removeIf(ei -> registry.getOrThrow(EnchantmentTags.CURSE).contains(ModUtil.enchantment(ei)));
                        itemEnchantments.forEach(ei -> maxedOutEnchantments.removeIf(ei1 -> !Enchantment.areCompatible(ModUtil.enchantment(ei1), ModUtil.enchantment(ei))));
                        if (maxedOutEnchantments.isEmpty()) {
                            Set<EnchantmentInstance> possibleUpgrades = itemEnchantments.stream().filter(ei ->
                                    (ModUtil.enchantment(ei).value().getMaxLevel() > 1) &&
                                    (ModUtil.level(ei) <= ModUtil.enchantment(ei).value().getMaxLevel()) &&
                                    ModUtil.enchantmentEligible(ModUtil.enchantment(ei))
                            ).collect(Collectors.toSet());
	...
	public static ItemStack applyPinnacleUpgrade(ItemStack original, Slot slot, AbstractContainerMenu container, NonNullList<Slot> slots) {
        if (container instanceof SmithingMenu sm && ModUtil.isPinnacleEnchantmentRecipe(slots) && slot.equals(slots.get(3))) {
            ...
            List<EnchantmentInstance> existingPinnacleEnchantments = original.getEnchantments().entrySet()
                    .stream().map(entry -> new EnchantmentInstance(entry.getKey(), entry.getIntValue()))
                    .filter(ei -> ModUtil.level(ei) > ModUtil.enchantment(ei).value().getMaxLevel())
                    .toList();
            if (existingPinnacleEnchantments.size() >= SSO.CONFIG.pinnacleEnchantment.maxPinnacleEnchantmentsOnItem.get()) {
                ...
                EnchantmentHelper.updateEnchantments(original, mutable -> mutable.set(
                        ModUtil.enchantment(toDowngrade),
                        ModUtil.enchantment(toDowngrade).value().getMaxLevel()
                ));
            }
            ...
            possibleUpgrades.removeIf(ei ->
                    (ModUtil.enchantment(ei).value().getMaxLevel() == 1) ||
                    !ModUtil.enchantmentEligible(ModUtil.enchantment(ei)) ||
                    existingPinnacleEnchantments.stream().anyMatch(ei1 -> ModUtil.enchantment(ei).equals(ModUtil.enchantment(ei1)))
            );
            ...
            EnchantmentHelper.updateEnchantments(original, mutable ->
                    mutable.upgrade(ModUtil.enchantment(toUpgrade), ModUtil.level(toUpgrade) + 1)
            );
