...
import net.minecraft.world.item.crafting.CraftingBookCategory;import org.jetbrains.annotations.Nullable;
*///?}
...
public class PortableItemRepairRecipe extends CustomRecipe {
    ...
    //?} else {
    /*public PortableItemRepairRecipe() {
    ...
    *///?}
    ...
    private boolean processRepair(CraftingInput input) {
		...
		boolean bl = !repairMaterial.isEmpty() && ModUtil.count(repairMaterial) <= unitsToMaxRepair + 1 && (flintCount == 0 || (flintMaterialValid && flintCount >= ModUtil.count(repairMaterial)));
		flintCount = 0;
        return bl;
    }

    @Override
    public @NotNull ItemStack assemble(@NotNull CraftingInput input/*? <26.1 {*//*, @Nullable HolderLookup.Provider registries*//*?}*/) {
        ItemStack outputItem = itemToRepair.copy();
        outputItem.setDamageValue(
                outputItem.getDamageValue() - (Mth.ceil((float) outputItem.getMaxDamage() / unitCost) * ModUtil.count(repairMaterial))
        );
    ...
    public @NotNull NonNullList<ItemStack> getRemainingItems(@NotNull CraftingInput input) {
		...
		int count = ModUtil.count(repairMaterial);
